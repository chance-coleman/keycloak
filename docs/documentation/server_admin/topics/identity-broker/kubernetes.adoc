...
different issuer URL. In this case, configure the in-cluster discovery URL. If the well-known configuration returns a
different issuer, {project_name} stores the discovered issuer for client assertion validation.
...
|The issuer URL of service account tokens, or the Kubernetes discovery URL for managed clusters. The URL `<ISSUER>/.well-known/openid-configuration` must be available to {project_name}
